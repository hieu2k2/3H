      ...
      for (let i = 0; i < data.content.length; i++) {
        if (data.content[i].state == true) {
          html1 += `<tr>
            ...
            <td>
            <button class="btn btn-outline-secondary" onclick="showDetail('${data.content[i].id}')">Detail</button>
            <button class="btn btn-outline-secondary mr-1" onclick="showEditProduct('${data.content[i].id}')">Edit</button>
            <Button class="btn btn-outline-danger" onclick="deleteProduct(${data.content[i].id},'${data.content[i].name}')">Del</Button></td></tr>`;
        }
      }
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      let html =
        ...
        "  </div>\n" +
        '  <div class="form-group row">\n' +
        '    <label for="score" class="col-sm-4 col-form-label">Brand</label>\n' +
        '    <div class="col-sm-8">\n' +
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      let html =
        ...
        '    <label for="age" class="col-sm-4 col-form-label"></label>\n' +
        '    <div class="col-sm-8">\n' +
        '      <button class="btn btn-outline-primary mt-2" type=\'button\' onclick="editColor(' + id + ')">Edit</button>' +
        "    </div>\n" +
        "  </div>\n" +
        "</form>" +
        "</div>";
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      let html =
        ...
        "    </div>\n" +
        '  <div class="form-group row">\n' +
        '    <label for="score" class="col-sm-4 col-form-label">TotalAmount</label>\n' +
        '    <div class="col-sm-8">\n' +
        '      <input type="text" class="form-control" name="TotalAmount" value=\'' +
        data?.totalAmount +
        "'>\n" +
        "    </div>\n" +
        "    </div>\n" +
        '  <div class="form-group row">\n' +
        '    <label for="score" class="col-sm-4 col-form-label">TotalPrice</label>\n' +
        '    <div class="col-sm-8">\n' +
...
function deleteOrder(id, name) {
  if (confirm("Do you want to delete " + name + "???")) {
    $.ajax({
      ...
      type: "Delete",
      url: API + "/orders/" + id,
      success: loadODerManagerContent,
      error: function (error) {
        console.log(error);
      },
    });
